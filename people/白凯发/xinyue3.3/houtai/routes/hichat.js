...
router.post('/new_comment', (req, res) => {
  ...
  req.on('data', function (data) {
    ...
    str=str.comment;
    console.log(str);
    img=str.img; name=str.name;text=str.text;agree=str.agree;color=str.color;zanguo=str.zanguo;
   console.log(img ,name, text ,agree,color,zanguo);

  });
  req.on('end', function () {
    db.query("insert into comment1_table(img,name,text,agree,color,zanguo) values('"+img+"', '"+name+"', '"+text+"', '"+agree+"' ,'"+color+"', '"+zanguo+"')", (err, data) => {
      if (err) {
        console.log(err);
        res.status(500).send('database error').end();
      } else {
...
router.post('/dianzan', (req, res) => {
  ...
  req.on('end', function () {
    db.query("update comment1_table set agree=agree + 1 , color='green',zanguo='true' WHERE ID = " + id + " ", (err, data) => {
      if (err) {
        console.log(err);
        res.status(500).send('database error').end();
      } else {
