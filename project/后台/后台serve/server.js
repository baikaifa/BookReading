...
function from_submit() {
    ...
    //获取评论表信息
    app.get('/comment_get', urlencodedParser, function (req, res) { //get
        comment1_table(req, res);
    });
    //删除评论
    app.post('/del_comment', urlencodedParser, function (req, res) { //post处理方法
        console.log( req.body.id,res);
        del_comment(req.body.id); //数据库方法，加入了两个参数，是提交的数据
        res.send('1');
        res.end();
    })
    //添加评论
    app.post('/make_comment', urlencodedParser, function (req, res) { //post处理方法
        make_comment(req.body.name,req.body.text,req.body.agree,res);
    });
}
...
//查找评论
function comment1_table(req, res) {
    var mysql = require("mysql");
    var connection = mysql.createConnection({ //配置参数，然后添加你的数据库里面的表
        host: 'localhost',
        user: 'root',
    ...
    connection.connect(); //连接
    var querySql = "SELECT * FROM  comment1_table  ";
    connection.query(querySql, function (err, result) {
        if (err) {
            console.log("[query error]-", err.message);
            return;
...
function del_comment(id,res) {
    ...
    connection.connect(); //连接
    var delSql = "DELETE FROM  comment1_table  WHERE ID = '" + id + "'"; //存放数据库语言的，这里是添加
    connection.query(delSql, function (err, res) {
        if (err) {
            console.log("[delete error]-", err.message);
            return;
...
//添加评论
function make_comment(name,text,agree, res) {
    console.log(name,text,agree);
    var mysql = require("mysql");
    var connection = mysql.createConnection({ //配置参数，然后添加你的数据库里面的表
        host: 'localhost',
        user: 'root',
    ...
    connection.connect(); //连接
    var querySql = "insert into comment1_table(name,text,agree) values('" + name + "','" + text + "','" + agree +"')  ";
    connection.query(querySql, function (err, result) {
        if (err) {
            console.log("[query error]-", err.message);
            return;
